...
public class ClientSnake extends Snake<SlytherClient> {
    ...
    public boolean update(float delta, float lastDelta, float lastDelta2) {
        ...
        if (turnDirection == 1) {
            ...
            float h = (float) ((wantedAngle - angle) % Snake.PI_2);
            if (h < 0) {
                h += Snake.PI_2;
            }
            if (h > Math.PI) {
        ...
        if (!dead) {
            if (ehang != wehang) {
                float h = (float) ((wehang - ehang) % Snake.PI_2);
                if (h < 0) {
                    h += Snake.PI_2;
                }
                if (h > Math.PI) {
                ...
                if (point.eiu > 0) {
                    ...
                    for (int i = cm; i >= 0; i--) {
                        ...
                        if (h >= Snake.HFC) {
                            ...
                            fx += point.exs[i] * Snake.HFAS[h];
                            fy += point.eys[i] * Snake.HFAS[h];
                        }
                    }
                    point.fx = fx;
                    point.fy = fy;
