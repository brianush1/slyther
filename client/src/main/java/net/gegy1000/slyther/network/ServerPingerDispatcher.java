...
public class ServerPingerDispatcher implements Runnable {
    ...
    @Override
    public void run() {
        ServerMan.INSTANCE.serversAvailable.lock();
        List<ServerMan.Server> servers = ServerMan.INSTANCE.getServerList();
        List<ServerPinger> pingers = new ArrayList<>();
        BlockingQueue<Pinger> finishedPingers = new LinkedBlockingQueue<>();
        if (servers.size() > 0) {
            for (ServerMan.Server server : servers) {
                try {
            ...
            try {
                ...
                while ((pinger = finishedPingers.take()) != POISON) {
                    ...
                    if (pinged == MIN_FOR_PLAY) {
                        ServerMan.INSTANCE.serversAvailable.unlock();
    ...
    private class ServerPinger extends WebSocketClient implements Pinger {
        private ServerMan.Server server;
        ...
        public ServerPinger(ServerMan.Server server, BlockingQueue<Pinger> finishedPingers) throws URISyntaxException {
            super(new URI("ws://" + server.getClusterIp() + ":80/ptc"), new Draft_6455(), ServerMan.INSTANCE.getHeaders(), 0);
            this.server = server;
