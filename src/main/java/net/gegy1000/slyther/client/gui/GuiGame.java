...
//				drawString(s, 10, debugY, 0.5F, 0xFFFFFF);
				...
				int mX = (int)client.mouseX - client.frameBufferWidth / 2;
				int mY = client.frameBufferHeight - (int)client.mouseY - client.frameBufferHeight/2;
				int dist = mX*mX+mY*mY;
				s = "mouseX=" + df3.format(mX) + " mouseY=" + df3.format(mY) 
				+ " wangle=" + df3.format(client.player.wantedAngle) + " dist=" + df3.format(dist);
				drawString(s, 10, debugY, 0.5F, 0xFFFFFF);
			}
			int XSIZE = 10;
			int XP = (int) (client.controller.getMouseX()*2 + renderResolution.getWidth() / 2.0);
			int YP = (int) (client.controller.getMouseY()*2 + renderResolution.getHeight() / 2.0F);
			drawLine(XP-XSIZE, YP-XSIZE, XP+XSIZE, YP+XSIZE, 3, 0xFFFFFF);
			drawLine(XP-XSIZE, YP+XSIZE, XP+XSIZE, YP-XSIZE, 3, 0xFFFFFF);
		} catch (Exception ex) {
